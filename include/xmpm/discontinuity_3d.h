...
class Discontinuity_3D : public DiscontinuityBase<Tdim> {
 ...
 public:
  ...
  //initialization
  virtual bool initialize(const std::vector<VectorDim>& coordinates,const std::vector<std::vector<mpm::Index>>& pointsets)
  {
    bool status = true;
    ...
    if (!point_status) {
      status = false;
      throw std::runtime_error("Addition of points in discontinuity to mesh failed");
    }
      // Create elements from file
    bool element_status =  create_elements(pointsets);
    if (!element_status) {
      status = false;
      throw std::runtime_error("Addition of elements in discontinuity to mesh failed");
    }

    bool normal_status =  initialize_center_normal();
    if (!normal_status) {
      status = false;
      throw std::runtime_error("initialized the center and normal of the discontunity failed");
    }
    return status;
  };

  //! create elements from file
  virtual bool create_elements(const std::vector<std::vector<mpm::Index>>& elements) override;

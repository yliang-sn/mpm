...
template <unsigned Tdim>
bool mpm::ParticleXMPM<Tdim>::assign_material_state_vars(
...
template <unsigned Tdim>
bool mpm::ParticleXMPM<Tdim>::assign_cell(
...
// Assign a cell to particle
template <unsigned Tdim>
bool mpm::ParticleXMPM<Tdim>::assign_cell_xi(
    const std::shared_ptr<Cell<Tdim>>& cellptr,
...
template <unsigned Tdim>
bool mpm::ParticleXMPM<Tdim>::assign_cell_id(mpm::Index id) {
  bool status = false;
  try {
...
template <unsigned Tdim>
void mpm::ParticleXMPM<Tdim>::remove_cell() {
...
template <unsigned Tdim>
bool mpm::ParticleXMPM<Tdim>::assign_material(
...
template <unsigned Tdim>
bool mpm::ParticleXMPM<Tdim>::compute_reference_location() noexcept {
...
template <unsigned Tdim>
void mpm::ParticleXMPM<Tdim>::compute_shapefn() noexcept {
...
template <unsigned Tdim>
bool mpm::ParticleXMPM<Tdim>::assign_volume(double volume) {
  bool status = true;
  try {
    if (volume <= 0.)
...
template <unsigned Tdim>
void mpm::ParticleXMPM<Tdim>::compute_volume() noexcept {
  // Check if particle has a valid cell ptr
  assert(cell_ != nullptr);
...
template <unsigned Tdim>
void mpm::ParticleXMPM<Tdim>::update_volume() noexcept {
...
template <unsigned Tdim>
void mpm::ParticleXMPM<Tdim>::compute_mass() noexcept {
...
template <>
inline Eigen::Matrix<double, 6, 1> mpm::ParticleXMPM<1>::compute_strain_rate(
...
// Compute strain rate of the particle
template <>
inline Eigen::Matrix<double, 6, 1> mpm::ParticleXMPM<2>::compute_strain_rate(
    const Eigen::MatrixXd& dn_dx, unsigned phase) noexcept {
  // Define strain rate
  Eigen::Matrix<double, 6, 1> strain_rate = Eigen::Matrix<double, 6, 1>::Zero();
...
// Compute stress
template <unsigned Tdim>
void mpm::ParticleXMPM<Tdim>::compute_stress() noexcept {
...
template <unsigned Tdim>
bool mpm::ParticleXMPM<Tdim>::assign_velocity(
...
template <unsigned Tdim>
bool mpm::ParticleXMPM<Tdim>::map_pressure_to_nodes() noexcept {
  // Mass is initialized
  assert(mass_ != std::numeric_limits<double>::max());
...
template <unsigned Tdim>
bool mpm::ParticleXMPM<Tdim>::compute_pressure_smoothing() noexcept {
  // Assert
  assert(cell_ != nullptr);
...
template <unsigned Tdim>
void mpm::ParticleXMPM<Tdim>::append_material_id_to_nodes() const {
  for (unsigned i = 0; i < nodes_.size(); ++i)
...
template <unsigned Tdim>
void mpm::ParticleXMPM<Tdim>::assign_neighbours(
